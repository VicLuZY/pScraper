...
func run(args []string, stdout io.Writer) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: permit-db import-jsonl --jsonl data/permits-db --sqlite data/permits.sqlite")
	}
	switch args[0] {
	case "import-jsonl":
...
func runImportJSONL(args []string, stdout io.Writer) error {
	...
	var reset bool
	fs := flag.NewFlagSet("permit-db import-jsonl", flag.ContinueOnError)
	...
	sum := importSummary{JSONLDir: jsonlDir, SQLite: sqlitePath, Reset: reset}
	currentPath := filepath.Join(jsonlDir, "current.jsonl")
	if err := readJSONL(currentPath, func(r model.PermitRecord) error {
	...
	if err := readOptionalJSONL(historyPath, func(evt model.StatusEvent) error {
	...
	if err := readOptionalJSONL(auditPath, func(a model.ScrapeAudit) error {
...
func readOptionalJSONL[T any](path string, fn func(T) error) error {
	if _, err := os.Stat(path); errors.Is(err, os.ErrNotExist) {
		return nil
	}
	return readJSONL(path, fn)
}
