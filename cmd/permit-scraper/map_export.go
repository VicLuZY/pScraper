...
func runMapExport(args []string, stdout io.Writer) error {
	var dbDir, webDir, outDir string
	var auditLimit int
	fs := flag.NewFlagSet("pScraper export-map", flag.ContinueOnError)
	...
	webFS := openPortableWebFS(webDir)
	if err := writePortableStaticIndex(webFS, filepath.Join(outDir, "index.html")); err != nil {
		return err
	}
	for _, name := range []string{"app.js", "styles.css"} {
		if err := copyPortableFile(webFS, name, filepath.Join(outDir, name)); err != nil {
			return err
		}
	}
	if err := writePortableDataJS(filepath.Join(outDir, "data.js"), payload); err != nil {
		return err
	}

	sum := portableExportSummary{
		Out:         outDir,
		DBPath:      dbDir,
		Records:     len(records),
		AuditRows:   len(audit),
...
func openPortableWebFS(webDir string) fs.FS {
	if webDir != "" {
...
func writePortableStaticIndex(webFS fs.FS, dst string) error {
...
func writePortableDataJS(path string, payload portableExportData) error {
...
func copyPortableFile(webFS fs.FS, name, dst string) error {
